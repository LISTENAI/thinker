#undef __OP__
#define __OP__ iqSub
#include "core/operator_attrs.h"
#include "core/operator_register.h"
#include "thinker_status.h"

#ifdef THINKER_USE_VENUS
#include "./venus/iqsub.h"
#endif

#ifdef THINKER_USE_ARCS
#include "./arcs/iqsub.h"
#endif

#ifdef THINKER_USE_VENUSA
#include "./venusA/iqsub.h"
#endif

/**
 * Forward pass implementation for Integer Quantized Subtraction operator
 * Performs element-wise subtraction of two quantized tensors
 * @param op: Operator structure containing binary operation attributes
 * @param tensors: Array of input/output tensors (tensor1, tensor2, output, workspace)
 * @param num_tensor: Total number of tensors
 * @param list: DMA list (unused)
 * @return: Status code indicating success or failure
 */
int32_t X(Forward)(tOperator *op, tTensor **tensors, int32_t num_tensor, tDMA_List *list) {
    // Workspace is optional when both inputs and the output can be used directly.
    CHECK_GE(num_tensor, (op->num_input_ + op->num_output_));
    
    // Get binary operation attributes
    iqBinaryAttrs *attrs = (iqBinaryAttrs *)((int8_t *)op + op->attr_offset_);
    
    tTensor *temp = NULL;
    if (num_tensor == op->num_input_ + op->num_output_ + 1) {
        temp = tensors[op->num_input_ + op->num_output_];
    }
    
#if THINKER_USE_VENUS || THINKER_USE_ARCS || THINKER_USE_VENUSA
#if THINKER_PROFILE
    uint64_t start_t = tick_count();
#endif

    // Call hardware-specific subtraction implementation
    THINKER_RET_CHECK(iqsub_luna(tensors[0], tensors[1], temp, tensors[op->num_input_]), "issub_luna");

#if THINKER_PROFILE
    uint64_t finish_t = tick_count();
    uint32_t total_t = (uint32_t)(finish_t - start_t);
    printf("%8s | %u | (","iqSub", total_t);  
#endif
#endif

    return T_SUCCESS;
}

#include "core/operator_template.h"
#undef __OP__
