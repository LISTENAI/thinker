// Swish operator implementation (β=1)
...
/**
 * @brief Execute the Swish operation
...
#ifdef THINKER_USE_VENUSA
    THINKER_RET_CHECK(swish_luna(tensors[0], tensors[op->num_input_], workspace), "swish_luna");
#endif
#if THINKER_PROFILE
    uint64_t finish_t = tick_count();
    uint32_t total_t = (uint32_t)(finish_t - start_t);
    printf("%8s | %u | (", "swish", total_t);
#endif
    return T_SUCCESS;
}
