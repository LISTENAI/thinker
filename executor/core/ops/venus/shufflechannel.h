 ...
 * @param X Input tensor in NCHW format
 * @param Y Output tensor in NCHW format
 * @param group Number of groups to shuffle
 */
    ...
    for (int32_t b = 0; b < batch_num; ++b) {
        ...
        for (int32_t i = 0; i < group; ++i) {
            for (int32_t j = 0; j < group_column; ++j) {
                const int8_t *p_i = input_ptr + (i * group_column + j) * length * X->byte_;
...
/**
 * @brief Channel shuffle operation for NHWC data format
 * @param X Input tensor in NHWC format
 * @param Y Output tensor in NHWC format
 * @param group Number of groups to shuffle
 */
...
/**
 * @brief Main function for channel shuffle operation
 * @param X Input tensor
 * @param Y Output tensor
 * @param attr Shuffle channel attributes containing group and axis
