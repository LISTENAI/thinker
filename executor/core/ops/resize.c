...
/**
 * @brief Coordinate transformation modes for resizing
 */
typedef enum coordinate_transformation_mode {
    khalf_pixel = 0,          // Half pixel mode
    kpytorch_half_pixel,     // PyTorch half pixel mode
    kalign_corners,          // Align corners mode
    kasymmetric,             // Asymmetric mode
    ktf_crop_and_resize      // TensorFlow crop and resize mode
} ctmode;
 ...
 */
typedef enum {
    knearnest = 0,  // Nearest neighbor mode
    klinear,       // Linear interpolation mode
    kcubic         // Cubic interpolation mode
} ResizeMode;

/**
 * @brief Nearest neighbor modes
 */
typedef enum {
    kround_prefer_floor = 0,  // Round prefer floor mode
    kround_prefer_ceil,       // Round prefer ceil mode
    kfloor,                   // Floor mode
    kceil                     // Ceil mode
} nearestMode;

/**
 * @brief Input/output indices for resize operation
 */
enum resize_io {
    kData = 0,  // Input data
    kRio,       // Region of interest
    kScales,    // Scale factors
    kSizes,     // Output sizes
    kOut        // Output data
};
 ...
 * @param mode Coordinate transformation mode
 * @param scale Scale factor
 * @param length_original Original length
 * @param length_resized Resized length
 * @param start_x Start coordinate
 * @param end_x End coordinate
 * @return float Transformed coordinate
 */
static float GetCoordinateFunc(int32_t x, ctmode mode, float scale,
    ...
    switch (mode) {
        ...
        case ktf_crop_and_resize:
            if (length_resized > 1) {
                x_original = start_x * (length_original - 1) +
                            x_resized * (end_x - start_x) *
                                (length_original - 1) / (length_resized - 1);
            } else {
                x_original = 0.5 * (start_x + end_x) * (length_original - 1);
            }
            break;
        default:
...
/**
 * @brief Get nearest pixel function
 * @param x Input coordinate
 * @param nmode Nearest neighbor mode
 * @return int32_t Nearest pixel coordinate
 */
static int32_t GetNearestPixelFunc(float x, nearestMode nmode) {
    switch (nmode) {
...
/**
 * @brief Resize nearest neighbor function for 4D float tensors
...
static float Bicubic(float x, float a) 
{
  x = fabs(x);
  if (x <= 1)
