...
// Lookup table for reciprocal square root values in Q15 format
...
/**
 * @brief Performs floor operation with +0.5 bias for integer values
 * @param x Input value
 * @param shift Right shift amount
 * @return Result after applying floor(x + 0.5) operation
 */
...
/**
 * @brief Calculates reciprocal square root using lookup table
 * @param data Input data
 * @param q_x Scale factor for input
 * @param table_shift Output shift value
 * @return Approximated reciprocal square root value
 */
static const int16_t calc_sqrt_reciprocal(const int64_t data, int32_t q_x, int32_t *table_shift) {
    const int q_normal = 10;    // Normalization range (-32, 32)
    const int q2 = 14;
    int64_t temp;
    int q1;

    // Determine normalization based on bit patterns
    if (data & 0xC00000000000) { temp = data>>38; q1 = 24; }
...
/**
 * @brief Layer normalization implementation for integer tensors
