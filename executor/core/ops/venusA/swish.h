...
static int32_t swish_scale_i32_to_q27(const int32_t *src, int32_t *dst, uint32_t size, int32_t shift) {
    int32_t scalar = 1;
    uint32_t rshift = 0;
    if (shift >= 0) {
...
static int32_t swish_scale_i32_to_i8(const int32_t *src, int8_t *dst, uint32_t size, int32_t shift) {
    int32_t scalar = 1;
    uint32_t rshift = 0;
    if (shift >= 0) {
#if THINKER_PARAM_CHECK
...
int32_t swish_luna(tTensor *X, tTensor *Y, tTensor *Temp) {
    ...
    if (X->dtype_ == Int8) {
        ...
        THINKER_RET_CHECK(swish_scale_i32_to_q27(tmp1, tmp1, input_size, shift_i), "swish_scale_i32_to_q27");
        THINKER_RET_CHECK(API_LIB(swish_i32o32)(tmp1, tmp1, input_size), "luna_swish_i32o32");
        THINKER_RET_CHECK(swish_scale_i32_to_i8(tmp1, dst, input_size, shift_o), "swish_scale_i32_to_i8");
    }
        ...
        THINKER_RET_CHECK(swish_scale_i32_to_q27(tmp, tmp, input_size, shift_i), "swish_scale_i32_to_q27");
        THINKER_RET_CHECK(API_LIB(swish_i32o32)(tmp, tmp, input_size), "luna_swish_i32o32");
        THINKER_RET_CHECK(swish_scale_i32_to_i8(tmp, dst, input_size, shift_o), "swish_scale_i32_to_i8");
    }
        ...
        if (shift_i != 0) {
            THINKER_RET_CHECK(swish_scale_i32_to_q27(src, tmp, input_size, shift_i), "swish_scale_i32_to_q27");
            src = tmp;
        }
