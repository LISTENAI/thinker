...
class MemoryPlaner(object):
    def __init__(self, graph, dma_prefetch: bool = True):
        ...
        # add workspace
        self.get_workspace(graph)
        if dma_prefetch:
            self.get_dma_buffer(graph)
    ...
    def get_workspace(self, graph):
        max_workspace = {}
        # new_workspace = {}
        life_begin = len(graph.nodes.values())
        life_end = 0
        for node in graph.nodes.values():
            workspace = node.op.get_workspace()
            ...
            tensor = workspace[0]
            if tensor.mem_type not in max_workspace:
                max_workspace[tensor.mem_type] = tensor.nbytes
            else:
                if max_workspace[tensor.mem_type] < tensor.nbytes:
                    max_workspace[tensor.mem_type] = tensor.nbytes
            ...
            # else:
            # new_workspace[MemType.SHARE_MEM] = max_workspace

        for k, v in max_workspace.items():
    ...
    def get_dma_buffer(self, graph):
        max_dma_buffer1 = {}
        max_dma_buffer2 = {}
        index = 0
        for node in graph.nodes.values():
            dma_size = 0
            if node.op_type in {"Conv1dInt", "Conv2dInt", "ConvTranspose2dInt", "LinearInt", "LSTMInt", "GRUInt", "LayerNormInt"}:
            # if node.op_type in {"Conv1dInt", "Conv2dInt", "ConvTranspose2dInt", "LinearInt", "LayerNormInt"} \
    ...
    def get_life_period(self):
        ...
        for i, _t in enumerate(self.entry_ctx_list):
            _t_prev = _t
            _t_same = [_t]
            while (_t_prev.share_id != _t_prev.entry.index and \
                _t_prev.entry.tensor.mem_type == self.entry_ctx_list[_t_prev.share_id].entry.tensor.mem_type):  
...
def memory_plan(graph: Graph, dma_prefetch_on: bool) -> Dict[int, List[int]]:
    plan_list = [
        MemoryGreedyBySize(graph, dma_prefetch_on),
        MemoryGreedyByBreadth(graph, dma_prefetch_on),
        MemoryGreedyByOrder(graph, dma_prefetch_on),
    ]
    ...
    for i in range(len(memory_list)):
        ...
        best_plan.mem_sizes[memory_list[i]] = list(map(int, best_plan.mem_sizes[memory_list[i]]))
        # print(
...
__all__ = ["WORKSPACE_NAME", "DMA_BUFFER1_NAME", "DMA_BUFFER2_NAME", "memory_plan", "get_memory_size",]
