...
class BatchNorm2dInt(Operator):
    ...
    def infer_tensor(self, dynamic_shape):
        ...
        platform = self.attrs.get("platform") or "venus"
        assert platform in ("venus", "arcs", "mars", "venusA"), \
            f"BatchNorm2dInt is not supported on {platform}"
        assert len(self.inputs) >= 3, "BatchNorm2dInt requires input, weight, and bias"
        X = self.inputs[0]
        assert len(X.shape) == 4, "Only 4D tensors are supported"
        assert all(is_sympy(dim) or dim > 0 for dim in X.shape), \
            "BatchNorm2dInt dimensions must be positive"
        ...
        assert self.inputs[1].size == X.shape[1], "Weight size must match input channels"
        assert self.inputs[2].size == X.shape[1], "Bias size must match input channels"
        ...
        assert 0 <= shift <= 63, \
            f"BatchNorm2dInt on {platform} requires Luna shift in [0, 63]"
    ...
    def get_workspace(self) -> List[Tensor]:
        ...
        X = self.inputs[0]
        assert X.mem_type == MemType.SHARE_MEM, "BatchNormInt requires SHARE_MEM input"
        assert self.outputs[0].mem_type == MemType.SHARE_MEM, "BatchNormInt requires SHARE_MEM output"
        workspace_factor = 6 if self.attrs.get("platform") == "venusA" else 4
        workspace_bytes = ALIGN4(X.shape[2] * X.shape[3] * workspace_factor)
