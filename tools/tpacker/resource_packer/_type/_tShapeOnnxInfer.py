...
class tShapeOnnxInferHdr(object):
    def __init__(
        self,
        dy_axis_list,
        id_pair_list,
        ...
        self.hdr = tffi.new("tShapeOnnxInferHdr *")
        ...
        # tensor_list
        self.hdr.tensor_list_offset_ = offset
        pack_buffer += tensor_list.to_bytes()
        offset = ALIGN16(len(pack_buffer))
        pack_buffer += b"\0" * (offset - len(pack_buffer))

        # input_ids
        self.hdr.inputs_ids_offset_ = offset
        for ids in input_ids:
            pack_buffer += ids.to_bytes()
        offset = ALIGN16(len(pack_buffer))
        pack_buffer += b"\0" * (offset - len(pack_buffer))
        self.hdr.num_inputs_ids_ = len(input_ids)

        # output_ids
        self.hdr.outputs_ids_offset_ = offset
        for ids in output_ids:
            pack_buffer += ids.to_bytes()
        offset = ALIGN16(len(pack_buffer))
        pack_buffer += b"\0" * (offset - len(pack_buffer))
        self.hdr.num_outputs_ids_ = len(output_ids)
...
__all__ = ["tUint", "tShapeOnnxInferHdr"]
