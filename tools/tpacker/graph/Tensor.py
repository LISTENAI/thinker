...
class Tensor:
    def __init__(self, shape, dtype, scale=-1.0, zero=0,
        layout      =Layout.NCHW, data=None, mem_type=None):
        self.shape  = tuple(shape)
        self.dtype  = np.dtype(dtype)
        self.bits   = np.float32(self.dtype.itemsize)
        self.scale  = scale
        self.zero   = zero
        self.layout = layout
        self.data   = data
    ...
    def copy(self):
        ...
        tensor.bits = self.dtype.itemsize
