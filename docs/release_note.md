...
- 新增多资源与流式仿真示例：增加 `demo/test_multi_resource` 与 `demo/test_stream_graph`，支持按 YAML 描述资源连接关系、KV cache 输入输出映射和逐 step 输出保存。
- 增强 C API 健壮性与资源检查：`tGetMemoryPlan`、`tModelInit`、`tCreateExecutor`、`tUpdateShape`、`tSetInput`、`tGetOutput` 等接口补充空指针、越界、shape infer 缺失与执行器内存校验，并修正参数区遍历和 DMA list 相关处理。
- 优化 Arcs / Venus / VenusA 算子实现：修复 `AvgPool2dInt`、`Concat`、`Conv2dInt`、`iqAdd`、`Split` 的 PSRAM 与 cache flush 处理，更新 `LayerNormInt` 的量化归一化逻辑，并补充 `LstmInt` 的 cell state 量化 `q_c` 支持。
- 更新 `tpacker` / `tvalidator`：修正内存参数解析，增强 `GLUInt`、`SoftmaxInt`、`LogSoftmaxInt` 的 `axis` 兼容和 `LstmInt` 的 `scale_c` 校验；`tvalidator` 改为按平台映射生成构建参数、清理工作目录并简化输入 dump 产物管理。
- 系统整理文档：同步 `README.md` 与 `README_EN.md`，重写/润色 `thinker_environment.md`、`thinker_build.md`、`thinker_compile.md`、`thinker_packer.md`、`thinker_run.md`、`thinker_validator.md`、`thinker_profile.md`、`thinker_performance.md`；将原 Docker 独立文档内容并入 `thinker_environment.md`，统一说明 x86/Linux 推理属于目标平台仿真验证，Thinker 上层代码在仿真平台与真实芯片平台通用，迁移时通常只需替换底层 `luna` 库和固件/BSP 库。
...
- 删除旧的 `tools/image_preprocess.py`，更新 `README.md` 与 `README_EN.md`。
